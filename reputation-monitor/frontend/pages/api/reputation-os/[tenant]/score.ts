/**
 * /api/reputation-os/[tenant]/score — Powered by the unified Processing Layer.
 *
 * Derives the reputation score from real-time Talk + Feed data instead of
 * proxying to a separate backend.
 */

import type { NextApiRequest, NextApiResponse } from "next";
import type { ReputationScore } from "@/lib/reputationOs";
import { ingestData, type DateFilter } from "@/lib/dataIngestion";
import { computeReputationScore } from "@/lib/reputationEngine";
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<ReputationScore | { error: string }>,
) {
  if (req.method !== "GET") {
